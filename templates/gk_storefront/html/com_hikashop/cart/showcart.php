...
<fieldset>
	<h1><?php if($cart_type == 'cart')echo JText::_('CARTS');else echo JText::_('WISHLISTS'); ?></h1>
	<div class="toolbar hikashop_header_buttons" id="toolbar" style="float: right;">
		<?php
			if($cart_type == 'cart' && $config->get('enable_multicart')){
		?>
				<a href="<?php echo hikashop_completeLink('cart&task=showcarts&cart_type='.$cart_type.$Itemid); ?>" class="button">
		...
		<?php echo $this->popup->display(
			...
			'hikashop_print_cart',
			760, 480, ' class="button"', '', 'link'
		); ?>

		<?php if($this->cartVal->display && ($userCurrent == $this->cartVal->user_id)){ ?>

			<a href="#" onclick="javascript:document.forms['hikashop_show_cart_form'].submit();" class="button">
		...
		?>
			<a href="#" class="button" onclick="history.back()">
		<?php
		}else{
		?>
			<a href="<?php echo JRoute::_('index.php?option='.HIKASHOP_COMPONENT.'&ctrl=cart&task=showcarts&cart_type='.$cart_type.$Itemid); ?>" class="button">
		<?php } ?>
