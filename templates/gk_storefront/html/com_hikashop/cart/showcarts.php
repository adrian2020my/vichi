...
<fieldset>
	...
	<div class="toolbar hikashop_header_buttons" id="toolbar" style="float: right;">
		...
		?>
				<a href="<?php echo hikashop_completeLink('cart&task=newcart&cart_type='.$cart_type.$Itemid_str); ?>" class="button">
		...
		<?php if($userCurrent != 0){ ?>
			<a href="<?php echo JRoute::_('index.php?option='.HIKASHOP_COMPONENT.'&view=user&layout=cpanel'.$Itemid_str); ?>" class="button">
...
<form action="<?php echo hikashop_completeLink('cart'); ?>" method="post"  name="adminForm" id="adminForm">
	<table class="hikashop_no_border">
		<tr>
			<td width="100%">
				<label id="hikashop_search_label"><?php echo JText::_( 'FILTER' ); ?>:
				<input type="text" name="search" id="hikashop_search" value="<?php echo $this->escape($this->pageInfo->search);?>" class="inputbox" onchange="document.adminForm.submit();" /></label>
