...
class GKTemplateSocial {
    ...
    public function socialApiParser($embed_mode = false) {
    	...
    	if($this->parent->API->get('fb_like', '0') == 1 && !$is_excluded) {
    		...
    		// clear FB like
    		GKParser::$customRules['/<div><gavern:social><fb:like(.*?)fb:like><\/gavern:social><\/div>/mi'] = '';
    	}
        // G+
    	if($this->parent->API->get('google_plus', '1') == 1 && !$is_excluded) {
    		...
    		// clear G+ button
    		GKParser::$customRules['/<div><gavern:social><g:plusone(.*?)g:plusone><\/gavern:social><\/div>/mi'] = '';
    	}
		// G+ Share button
		  if($this->parent->API->get('google_plus_share', '1') == 1 && !$is_excluded) {
             ...
             if($this->parent->API->get('google_plus_share_annotation', 'vertical-bubble') != 'vertical-bubble') {
              ...
              GKParser::$customRules['/<div><gavern:social><g:plus(.*?)g:plus><\/gavern:social><\/div>/mi'] = '';
           }
    	// Twitter
    	if($this->parent->API->get('tweet_btn', '0') == 1 && !$is_excluded && $option == 'com_content' && $view == 'article') {
    		...
    		GKParser::$customRules['/<div><gavern:social><a href="http:\/\/twitter.com\/share"(.*?)\/a><\/gavern:social><\/div>/mi'] = '';
    	}
		// Pinterest
        if($this->parent->API->get('pinterest_btn', '0') == 1 && !$is_excluded && $option == 'com_content' && $view == 'article') {
              ...
              GKParser::$customRules['/<div><gavern:social><a href="http:\/\/pinterest.com\/pin\/create\/button\/(.*?)\/a><\/gavern:social><\/div>/mi'] = '';
         }
